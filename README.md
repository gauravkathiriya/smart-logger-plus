...
```bash
npm install smart-logger-plus
# or
pnpm add smart-logger-plus
# or
yarn add smart-logger-plus
```
...
```ts
import { createLogger } from "smart-logger-plus";
...
app.use((req, res, next) => {
  req.logger = logger.withRequestId(
...
Logs are **always** written to the console _and_ appended to the file. ANSI color codes are automatically stripped from file output.
...
```js
// CommonJS
const { createLogger } = require("smart-logger-plus");
